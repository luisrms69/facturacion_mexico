...
def execute():
	...
	if frappe.db.exists("DocType", doctype_name):
		frappe.msgprint(frappe._(f"{doctype_name} ya existe. Se forzará recarga desde JSON."))
		frappe.db.set_value("DocType", doctype_name, "module", module)
	else:
		frappe.msgprint(frappe._(f"{doctype_name} no encontrado. Se cargará por primera vez."))
	...
	if not frappe.db.exists("DocType", "Factura Fiscal Mexico"):
		frappe.throw(frappe._("DocType padre 'Factura Fiscal Mexico' no existe."))

	# Verificación rápida de campo
	meta = frappe.get_meta("Factura Fiscal Mexico")
	if not meta.get_field("respuestas_facturapi"):
		frappe.msgprint(frappe._("El campo 'respuestas_facturapi' no está presente."))
	else:
		frappe.msgprint(frappe._("Referencia a tabla hija verificada correctamente."))
