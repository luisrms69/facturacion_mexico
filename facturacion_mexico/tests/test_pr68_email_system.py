...
class TestPR68EmailSystem(FrappeTestCase):
	...
	def test_send_invoice_email_success(self):
		...
		with patch("frappe.db.get_value", return_value=mock_company_settings):
			with patch.object(FacturAPIClient, "_get_api_key") as mock_get_key:
				...
				with patch.object(FacturAPIClient, "_make_request") as mock_make_request:
					...
					# Test method
					client = FacturAPIClient(company="Test Company")
	...
	def test_send_fiscal_email_success(self):
		...
		with patch(
			"facturacion_mexico.facturacion_fiscal.doctype.factura_fiscal_mexico.factura_fiscal_mexico._resolve_recipient_email"
		) as mock_resolve_email:
		...
		with patch(
			...
			with patch("frappe.logger"):
				...
				with patch(
					...
					# El logger fue llamado (no verificamos la variable, solo que no hubo excepción)
					pass
