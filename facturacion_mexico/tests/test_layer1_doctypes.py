...
class TestLayer1DocTypes(unittest.TestCase):
	...
	def test_facturacion_mexico_company_settings_doctype(self):
		"""Test: DocType Facturacion Mexico Company Settings"""
		self.assertTrue(frappe.db.exists("DocType", "Facturacion Mexico Company Settings"))
	...
	def test_uso_cfdi_sat_doctype(self):
		...
		uso_cfdi = frappe.db.get_value("Uso CFDI SAT", {"code": "G01"}, ["code", "description"], as_dict=True)
		if uso_cfdi:
	...
	def test_regimen_fiscal_sat_doctype(self):
		...
		# Verificar estructura de un registro
		regimen = frappe.db.get_value(
			"Regimen Fiscal SAT", {"code": "601"}, ["code", "description"], as_dict=True
		)
		if regimen:
	...
	def test_addenda_type_doctype_exists(self):
		...
		if addenda_type_exists:
			...
			for field in expected_fields:
				self.assertIn(field, field_names, f"Addenda Type debe tener campo {field}")
	...
	def test_doctype_meta_loading(self):
		...
		basic_doctypes = ["Facturacion Mexico Company Settings", "Uso CFDI SAT", "Regimen Fiscal SAT"]
	...
	def test_doctype_creation_basic(self):
		"""Test: Carga básica de DocType funciona"""
		try:
			meta = frappe.get_meta("Facturacion Mexico Company Settings")
			self.assertIsNotNone(meta)
		except Exception as e:
			self.fail(f"Error en carga básica de DocType: {e}")
