...
class TestLayer2AddendaTemplatesMultiSucursal(unittest.TestCase):
	...
	def test_addenda_template_doctype_structure(self):
		...
		self.assertGreater(
			len(available_doctypes), 0, "Al menos un DocType de template de addenda debe estar disponible"
		)
		...
		for doctype in available_doctypes:
			...
			template_fields = [
				f
				for f in fields
				if any(keyword in f.lower() for keyword in ["template", "xml", "content", "definition"])
			]
	...
	def test_addenda_template_branch_variables_support(self):
		...
		for doctype in template_doctypes:
			...
			meta = frappe.get_meta(doctype)
			doc_fields = [
				f for f in meta.fields if f.fieldtype in ["Text", "Long Text", "Code", "Text Editor"]
			]

			if doc_fields:
				print(
					f"✓ {doctype} tiene campos para templates/documentación: {[f.fieldname for f in doc_fields]}"
				)
		...
		for module_path in template_modules:
			try:
				...
				render_methods = [
					...
					if any(keyword in m.lower() for keyword in ["render", "process", "generate", "compile"])
				]
	...
	def test_addenda_template_branch_context_injection(self):
		...
		try:
			...
			context_methods = [
				...
				if any(keyword in m.lower() for keyword in ["context", "branch", "sucursal", "variable"])
			]
	...
	def test_template_validation_with_branch_fields(self):
		...
		self.assertGreater(len(all_branch_fields), 0, "Branch debe tener campos disponibles para templates")

		# Verificar tipos de campo útiles para templates
		useful_field_types = ["Data", "Link", "Select", "Text", "Long Text"]
		useful_fields = [f for f in all_branch_fields if f.fieldtype in useful_field_types]
	...
	def test_addenda_template_xml_structure_validation(self):
		...
		if frappe.db.exists("DocType", "Addenda Type"):
			...
			if test_addenda_types:
				...
				xml_fields = [
					f
					for f in addenda_meta.fields
					if f.fieldtype in ["Code", "Text Editor", "Long Text"]
					and any(keyword in f.fieldname.lower() for keyword in ["xml", "template", "content"])
				]
		...
		for module_path in mapping_modules:
			try:
				...
				mapping_attrs = [
					...
					if any(keyword in attr.lower() for keyword in ["mapping", "map", "field", "variable"])
				]
