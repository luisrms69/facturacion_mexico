...
class TestLayer1BasicInfrastructure(unittest.TestCase):
	...
	def test_facturacion_mexico_company_settings_doctype_exists(self):
		"""Test: Verificar que el DocType de configuración por empresa existe"""
		exists = frappe.db.exists("DocType", "Facturacion Mexico Company Settings")
		self.assertTrue(exists, "DocType Facturacion Mexico Company Settings debe existir")
	...
	def test_custom_fields_basic_existence(self):
		...
		expected_customer_fields = ["fm_uso_cfdi_default"]  # fm_regimen_fiscal migrado a tax_category nativo

		for field in expected_customer_fields:
			self.assertIn(field, customer_field_names, f"Customer debe tener custom field {field}")
