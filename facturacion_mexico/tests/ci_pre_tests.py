def run():
	...
	try:
		for add_type in ("TEST_AUTOMOTIVE", "TEST_RETAIL", "TEST_GENERIC"):
			if not frappe.db.exists("Addenda Type", add_type):
				frappe.get_doc({"doctype": "Addenda Type", "name": add_type, "description": add_type}).insert(
					ignore_permissions=True
				)
	except Exception as e:
		log(f"Addenda Types failed (non-critical): {e}")  # Skip if can't create

	# --- 9) Impuesto SAT — catálogo requerido por Link en Regla Impuesto CFDI Recibido ---
	for imp in (
		{"name": "001", "code": "001", "description": "ISR", "vigencia_desde": "2010-01-01"},
		{"name": "002", "code": "002", "description": "IVA", "vigencia_desde": "2010-01-01"},
		{"name": "003", "code": "003", "description": "IEPS", "vigencia_desde": "2010-01-01"},
	):
	...
	report = {
		...
		"warehouse_types": [
			wt
			for wt in ("Stores", "Work In Progress", "Finished Goods", "Transit")
			if frappe.db.exists("Warehouse Type", wt)
		],
		"uoms": [u for u in ("Nos", "Unit", "Piece") if frappe.db.exists("UOM", u)],
		"customer_groups": [
			g for g in ("All Customer Groups", "Individual") if frappe.db.exists("Customer Group", g)
		],
		"territories": [
			t for t in ("All Territories", "Rest Of The World") if frappe.db.exists("Territory", t)
		],
		"item_groups": [g for g in ("All Item Groups", "Products") if frappe.db.exists("Item Group", g)],
		"price_lists": [
			p
			for p in ("Standard Selling (INR)", "Standard Selling (MXN)")
			if frappe.db.exists("Price List", p)
		],
		"items_created": [i for i in ("Test Item Default", "_Test Item") if frappe.db.exists("Item", i)],
		"addenda_types": [
			a
			for a in ("TEST_AUTOMOTIVE", "TEST_RETAIL", "TEST_GENERIC")
			if frappe.db.exists("Addenda Type", a)
		],
	}
