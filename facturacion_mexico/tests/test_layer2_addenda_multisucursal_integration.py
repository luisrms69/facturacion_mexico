...
class TestLayer2AddendaMultiSucursalIntegration(unittest.TestCase):
	...
	def test_branch_addenda_custom_fields_integration(self):
		...
		self.assertGreaterEqual(
			len(fiscal_fields), 1, "Branch debe tener al menos un campo fiscal para integración con addendas"
		)
	...
	def test_sales_invoice_branch_addenda_integration(self):
		...
		self.assertGreater(len(multi_sucursal_fields), 0, "Sales Invoice debe tener campos de Multi-Sucursal")
		self.assertGreater(len(addenda_fields), 0, "Sales Invoice debe tener campos de Addenda")
	...
	def test_customer_branch_addenda_relationship(self):
		...
		self.assertGreater(
			len(customer_addenda_fields), 0, "Customer debe tener campos de addenda para integración"
		)
		...
		try:
			...
			self.assertTrue(
				has_customer_standard or has_customer_relation,
	...
	def test_addenda_generation_with_branch_context(self):
		...
		try:
			...
			branch_aware_methods = [
				m for m in generator_methods if "branch" in m.lower() or "sucursal" in m.lower()
			]
		...
		if len(cert_fields) > 0:
			...
			for field in cert_fields:
				self.assertIsNotNone(
					field.fieldtype, f"Campo de certificado {field.fieldname} debe tener tipo definido"
				)
		else:
			...
			for template_doctype in available_templates:
				try:
					...
					template_content_fields = [
						f for f in template_fields if "template" in f or "xml" in f or "content" in f
					]
	...
	def test_cross_module_validation(self):
		...
		self.assertEqual(
			len(field_names), len(unique_fields), "No debe haber campos duplicados en Sales Invoice"
		)
		...
		if multi_sucursal_sections and addenda_sections:
			print(
				f"✓ Secciones organizadas: Multi-Sucursal={multi_sucursal_sections}, Addenda={addenda_sections}"
			)
		...
		self.assertNotIn(
			None, [f.fieldname for f in si_fields], "Todos los custom fields deben tener nombres válidos"
		)
